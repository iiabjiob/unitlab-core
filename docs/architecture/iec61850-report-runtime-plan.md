...
Signal-list production execution no longer has a separate manual "Online 61850" preparation button. The Signal List UI starts IEC 61850 preparation only through Start Run; selected signal ids are submitted with the test-run job and the backend worker rebuilds target sources, subscription plan, endpoint catalog, runtime adapter choice, and diagnostics from the active signal-list/allocation context before physical test commands are sent.

An uploaded SCD is not required for this path. When selected signal rows contain only a transport endpoint such as `172.16.40.128:12447` plus an IEC 61850 address, the planner treats that row as a signal-list discovery fallback: it builds the MMS endpoint catalog from the row metadata, leaves the ReportControl unresolved, connects to the live MMS endpoint, discovers logical devices/DataSets/RCBs, and selects the discovered RCB whose DataSet members match the requested signal address. The planner must not derive a ReportControl name from the signal leaf, for example `stVal`; that address is the expected signal, not the subscription control.

If the live MMS endpoint cannot be reached or the persistent client fails during read/reserve/enable/GI activation, Online orchestration returns a degraded verification snapshot instead of propagating a FastAPI 500. The affected session is marked `failed`, discovery is marked `unavailable` when discovery never completed, each affected subscription is marked `failed`/`degraded`, and the recovery state carries the runtime diagnostic code such as `EXTERNAL_MMS_ENDPOINT_UNREACHABLE`.

Successful live discovery emits an `MMS_DISCOVERY_SUMMARY` diagnostic with endpoint host/port and discovered model counts. Production Signal List surfaces the compact Start Run preparation sequence in the global progress card; detailed IED state remains available through External IED discovery state and backend/runtime telemetry.

The previous frontend-driven Online 61850 modal/orchestration flow was removed from Signal List to avoid a second source of truth. Backend orchestration APIs may remain available for diagnostics or lower-level runtime tests, but the operator workflow is worker-owned Start Run preparation.

The Signal List External IED drawer has a separate manual inspector path for engineer diagnostics. Selecting a discovered ReportControl in the IED tree can call the backend `POST /api/v1/workspaces/{workspace_id}/verification/external-ieds/{endpoint}/reports/enable` or `/disable` endpoint. This path opens a scoped manual MMS client session for that endpoint/report and writes `RptEna` through the backend IEC 61850 control service; the frontend only marks the report as enabled after the backend confirms the operation. This manual inspector state is transient and must not be treated as production Start Run preparation, verification evidence, or a replacement for worker-owned subscriptions.

Signal test-run jobs can request IEC 61850 verification in their job payload. Because test-run execution happens in the worker process, the worker creates its own backend runtime orchestration from the same signal-list context, keeps those subscriptions open for the job duration, waits for a new report after each command, and records the report evidence against the test-run job id. There is no frontend-created Online orchestration id in the production Signal List path.

Start Run preparation is backend-owned. The frontend enables IEC 61850 verification when the selected rows contain wizard-confirmed IEC 61850 mappings; it does not need a pre-existing manual Online 61850 orchestration. Before sending any physical DO/AO commands, the signal test-run worker first checks that the UnitLab peripheral devices required to activate the selected signals are bound to executable DO/AO channels and online. Offline or invalid peripheral rows are reported as preparation warnings and later skipped as normal per-row test evidence; they do not block online rows in the same selection. IEC 61850 preparation is built only for the mapped rows whose UnitLab peripheral activation path is executable. The worker then reads the persisted External IED Discovery Planner signal results and builds the runtime subscription graph from matched FCDA -> DataSet -> RCB metadata. If IEC 61850 planning, report subscription, or GI fails, the worker records the preparation diagnostic and continues the physical test without validation. Rows without IEC 61850 mapping remain normal physical test steps and are not counted as failed IEC 61850 verification.

During test-run preparation, the worker publishes concise job-result telemetry (`verification_prepare_steps`) before physical commands start. The operator UI shows only the high-level sequence: Peripheral online, Subscribe reports, General interrogation, Start test. Detailed report subscription state remains backend/runtime telemetry rather than global-progress UI.
...
- the MMS adapter keeps one discovery control service per backend session, then isolates active live RCB subscriptions in per-ReportControl native control processes so one native client's selected-RCB state cannot corrupt another RCB subscription;
- subscription control processes are seeded from the shared discovery cache and use discovered MMS `domain:item` RCB paths for direct enable/GI/cleanup writes, avoiding repeated full-device discovery for every report; live Start Run preparation does not rewrite static `OptFlds`/`TrgOps` RCB configuration during subscription activation, because real IEDs may reject those writes;
- signal-list-only discovery planning groups unresolved IEC 61850 addresses by live logical-device domain plus functional constraint, for example `KINTE15BCU01CTRL2/ST`, instead of creating one subscription group per logical node or signal path; non-report-observable control/config scopes such as `CO` and system logical-device scopes are not turned into report subscriptions unless explicit ReportControl metadata is present;
- signal-list fallback groups use stable internal ReportControl references during discovery so multiple unresolved signal groups cannot overwrite each other's live RCB selection; live matching prefers exact DataSet-member matches, falls back to the requested MMS domain when member metadata is sparse, and groups that resolve to the same live DataSet/base RCB share one physical RptEna subscription;
- Online preparation reserves direct URCB subscriptions with `Resv=true` before `RptEna=true`, releases the reservation during cleanup, and requests startup GI after enabling each matched MMS report subscription so the IED sees a real GI command; startup does not add a second delayed wait after GI, so if no decoded report values are available immediately, the subscription remains reporting with a warning while later test-run capture waits for a fresh report event;
- orchestration subscription snapshots expose `gi_requested`, `last_report_value_count`, and a compact `last_report_values` payload so the operator UI can distinguish "GI sent, no report values yet" from "GI returned values";
...
This closes the previous diagnostic-only gap where a frontend-driven preparation action could iterate targets while the backend singleton only retained the most recent target state. The remaining production gap is test-trigger correlation: the prepared subscriptions must feed an awaited report-event window for each triggered FAT step instead of treating GI alone as final test evidence.
...
- Native MMS write handling returns per-item WriteResponse failures for unsupported signal writes, `GI=true` while `RptEna=false`, and static RCB configuration writes attempted while the BRCB is enabled; same-value static RCB writes remain accepted before enable for client compatibility.
- Native report diagnostics track queued, coalesced, dropped, sent, and high-watermark report queue state; queue overflow sets `BufOvfl` and logs the drop.
...
### Slice 13X - External IED Simulator Linked Client Metadata Smoke

Implemented in this slice:

- Added a linked-only CTest target that starts the libIEC61850-backed IED simulator in-process on `127.0.0.1`.
- The smoke test connects with libIEC61850's `IedConnection` client API and reads server metadata through the actual MMS path.
- The test verifies logical device discovery, DataSet directory discovery, DataSet member directory discovery, BRCB directory discovery, and BRCB values for `RptID`, `DatSet`, `ConfRev`, `BufTm`, and `IntgPd`.
- The test is compiled only when `UNITLAB_IEC61850_SIM_WITH_LIBIEC61850=ON`, so the unlinked simulator build remains independent from GPL/native simulator code.
- No backend adapter, public REST/WebSocket API, production MMS client, GOOSE, SV, or fake report subscription success is introduced.
...
- Native C compile, CMake configure/build, and CTest fixture-parser/model-plan/model-loader tests without libIEC61850 linked.
- Linked CMake configure/build and CTest fixture-parser/model-plan/model-loader/linked-client-smoke tests against a locally built libIEC61850 checkout.
- Linked `--smoke-start` validates server start/stop.

### Slice 13Y - Backend External Simulator Endpoint Readiness

Implemented in this slice:

- Backend external simulator startup now waits for the MMS endpoint TCP port to accept a connection after the process is spawned.
- Process liveness alone is no longer treated as endpoint readiness.
- If the endpoint does not become reachable before timeout, startup fails with `SIMULATOR_ENDPOINT_READY_TIMEOUT`.
- If the process exits before readiness, startup fails with `SIMULATOR_PROCESS_EXITED`.
- Failed readiness stops the spawned process before returning the error.
- The readiness check is still transport-level TCP readiness; it does not claim IEC 61850 MMS association or RCB metadata validation.
...
- Backend service tests cover TCP readiness probing, fail-closed timeout, process cleanup after readiness failure, process-plan cleanup, and wrapper cleanup.

### Slice 13Z - Backend External Simulator Metadata Probe

Implemented in this slice:

- The native simulator binary now supports `--metadata-probe` when built with libIEC61850.
- The probe connects to a running simulator endpoint with libIEC61850 client APIs and verifies logical devices, DataSets, DataSet member counts, BRCB/URCB directories, and RCB values for `RptID`, `DatSet`, `ConfRev`, `BufTm`, and `IntgPd`.
- Backend external simulator startup now runs this metadata probe after TCP readiness before treating the simulator as ready for runtime execution.
- Metadata probe failure fails closed with `SIMULATOR_METADATA_PROBE_FAILED` and stops the spawned simulator process.
- The backend still does not link libIEC61850; it invokes the external simulator binary as an internal test helper.
...
- Native linked CMake build and CTest validate the metadata probe against a real in-process libIEC61850 server.
...
### Slice 13AC - Backend Subscription-Plan Simulator GI Validation

Implemented in this slice:

- Backend runtime can validate a subscription plan directly against external IED simulator processes without requiring callers to manually build the process plan first.
- The helper writes the simulator fixture from the subscription plan, allocates simulator endpoints, starts all required simulator processes, runs metadata and GI probes, and stops the processes.
- The result returns the generated process plan, GI probe results, and process stop evidence for audit/debug workflows.
- This remains an explicit simulator validation path only; it does not execute production MMS subscriptions and does not add public REST/WebSocket API surface.

Still planned:

- Wire the backend MMS adapter contract to this simulator endpoint for actual runtime read/reserve/enable/GI.
- Add UI controls only after runtime ownership and operator gating are defined.

Validation:

- Backend service tests cover subscription-plan fixture creation, endpoint allocation, GI probe command sequence, and process cleanup.

### Slice 13AD - External IED Simulator All-Report GI Probe

Implemented in this slice:

- The native `--gi-probe` path now validates every ReportControl in the selected fixture device instead of only the first ReportControl.
- Each ReportControl is enabled independently, receives a GI request, validates report metadata, validates DataSet value count, validates the first fixture value, then is disabled before the next ReportControl is probed.
- The linked smoke fixture now includes both buffered and unbuffered ReportControls so the GI probe covers both paths through the real libIEC61850 server/client callback flow.
...
- Native linked CTest covers all-report GI probing through a real in-process libIEC61850 server.
- Native unlinked CTest still validates the fail-closed no-libIEC61850 path.

### Slice 13AE - External IED Simulator All-Value GI Probe

Implemented in this slice:

- The native GI probe now validates every value returned in each GI report against the fixture DataSet order and `initialValue` entries.
- Value validation now checks the expected primitive type for boolean, integer, real, and string fixture values and reports a deterministic probe error on parse or value mismatch.
- The linked smoke fixture now uses a multi-member DataSet so the real libIEC61850 callback path proves value ordering and value propagation beyond the first DataSet member.
- This is still simulator validation only; it does not execute production subscriptions and does not add a production MMS client.
...
- Native linked CTest covers all-value GI probing through a real in-process libIEC61850 server.
- Native unlinked CTest still validates the fail-closed no-libIEC61850 path.

### Slice 13AF - External IED Simulator GI DataRef Validation

Implemented in this slice:

- The native GI probe now validates report DataRef optional field values when the fixture ReportControl has `OptFlds.dataReference` enabled.
- Each DataRef is checked against the DataSetEntry variable generated from the fixture signal reference, so the probe verifies value order and signal identity, not only value count.
- Missing or mismatched DataRef values fail closed with deterministic probe error codes.
- This is still simulator validation only; it does not execute production subscriptions and does not add a production MMS client.
...
- Native linked CTest covers GI DataRef validation through a real in-process libIEC61850 server.
- Native unlinked CTest still validates the fail-closed no-libIEC61850 path.

### Slice 13AG - External IED Simulator GI Cleanup Validation

Implemented in this slice:

- The native GI probe now treats ReportControl cleanup as validation evidence instead of best-effort teardown.
- After each GI probe, the client disables the ReportControl, releases buffered ReportControl reservation through `ResvTms`, and re-reads the RCB state.
- Cleanup fails closed when the ReportControl remains enabled, when a buffered ReportControl remains reserved, or when cleanup writes/reads fail.
- This is still simulator validation only; it does not execute production subscriptions and does not add a production MMS client.
...
- Native linked CTest covers GI cleanup validation through a real in-process libIEC61850 server.
- Native unlinked CTest still validates the fail-closed no-libIEC61850 path.

### Slice 13AH - External IED Simulator GI Optional-Field Validation

Implemented in this slice:

- The native GI probe now validates configured optional-field presence for `SeqNum`, `TimeOfEntry`, `DatSet`, `ReasonForInclusion`, `ConfRev`, and `DataRef` when those fields are enabled in the fixture ReportControl.
- Missing configured optional fields fail closed with deterministic probe error codes before value and cleanup validation are considered successful.
- `BufOvfl` and `EntryID` are not treated as blocking in this probe path because the linked simulator/client path does not expose them consistently for the current GI validation fixture.
- This is still simulator validation only; it does not execute production subscriptions and does not add a production MMS client.
...
- Native linked CTest covers GI optional-field validation through a real in-process libIEC61850 server.
- Native unlinked CTest still validates the fail-closed no-libIEC61850 path.

### Slice 13AI - External IED Simulator RCB Mask/State Metadata Validation

Implemented in this slice:

- The native metadata probe now validates live RCB `TrgOps` and `OptFlds` values against the model-plan bit masks generated from the fixture.
- The metadata probe now fails closed if a ReportControl is already enabled or if a buffered ReportControl is still reserved through `ResvTms`.
- The linked smoke test cleanup now explicitly releases the BRCB reservation after its direct GI check, so metadata validation can prove a clean initial state before backend readiness treats the simulator as usable.
- This is still simulator validation only; it does not execute production subscriptions and does not add a production MMS client.
...
- Native linked CTest covers RCB mask/state metadata validation through a real in-process libIEC61850 server.
- Native unlinked CTest still validates the fail-closed no-libIEC61850 path.

### Slice 13AJ - External IED Simulator DataSet Member Metadata Validation

Implemented in this slice:

- The native metadata probe now validates DataSet member identity and order, not only member count.
- Each live DataSet member returned by `GetDataSetDirectory` is checked against the model-plan signal LN, object reference, and functional constraint.
- Mismatched member order or unexpected member reference fails closed with deterministic probe error codes.
- This is still simulator validation only; it does not execute production subscriptions and does not add a production MMS client.
...
### Slice 13AL - Targeted External IED Simulator GI Report Probe

Implemented in this slice:

- The native simulator CLI supports `--gi-probe --report-key KEY` to validate one fixture ReportControl instead of the whole selected IED.
- The targeted probe still performs the real linked client sequence for that RCB: read, reserve when buffered, enable, request GI, validate values/optional fields/DataRefs, disable, release, and cleanup-read.
- Unknown report keys fail closed with `IEC61850_GI_PROBE_REPORT_NOT_FOUND`.
- Backend GI probe helper can pass a report key and validates the targeted stdout contract, including `reportKey` and `reports=1`.
- This remains simulator validation only; it is not a production MMS subscription adapter.
...
- Native linked smoke test covers targeted GI probe success and unknown report-key failure.
- Backend service tests cover targeted GI command construction and malformed targeted stdout.

### Slice 13AM - Subscription-Plan Targeted GI Validation

Implemented in this slice:

- Backend process-plan validation can run targeted GI probes for each required ReportControl in a subscription plan.
- `validate_report_subscription_plan_with_external_ied_simulators()` now validates the same report set that the operator selected/planned, instead of probing only per IED.
- Process-plan endpoint resolution now exposes the matching simulator process spec so report-level validation can route each targeted probe to the correct IED endpoint.
- The older per-IED all-report GI validation helper remains available for broad simulator smoke checks.
- This remains simulator validation only; it does not add production MMS subscription execution.
...
- Backend service tests cover report-level GI probe routing and subscription-plan validation cleanup.
