...
Successful live discovery emits an `MMS_DISCOVERY_SUMMARY` diagnostic with endpoint host/port and discovered model counts. The Online 61850 preparation UI uses this diagnostic to show real logical-device/DataSet/ReportControl counts and to keep failed-step messages tied to the runtime stage that actually failed.

The Online 61850 frontend call uses a dedicated orchestration timeout instead of the generic 30-second HTTP timeout, because live MMS discovery plus report enable/GI may legitimately take longer on large IED models. Operator cancel still aborts the waiting request; the backend remains responsible for stopping active orchestration sessions through the explicit stop action.
...
- subscription control processes are seeded from the shared discovery cache and use discovered MMS `domain:item` RCB paths for direct enable/GI/cleanup writes, avoiding repeated full-device discovery for every report; live Online 61850 does not rewrite static `OptFlds`/`TrgOps` RCB configuration during subscription activation, because real IEDs may reject those writes;
