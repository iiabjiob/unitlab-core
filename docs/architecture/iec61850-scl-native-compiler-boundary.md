...
Current implemented compiler coverage is intentionally narrow: selected IED, AccessPoint, Server, LDevice, LN0/LN, DataSet FCDA/FCD members, ReportControl, TrgOps, OptFields, and minimal DataTypeTemplates lookup for FCDA typed defaults, including direct `DAType/BDA` nested attribute paths and first-value `EnumType` defaults. Invalid FCDA/FCD members are reported with `SCL_DATASET_MEMBER_INVALID` and are not emitted as runtime signals. ReportControl objects referencing missing datasets are reported with `SCL_REPORT_DATASET_MISSING` and are not emitted as runtime RCBs. Diagnostics carry C-readable context fields for IED, AccessPoint, logical device, logical node, DataSet, ReportControl, and member reference where available. SCL XML is parsed through vendored `pugixml`; malformed XML returns `SCL_XML_PARSE_FAILED` diagnostics instead of an empty silent model.

## Next Compiler Slices

1. Split the current `pugixml` DOM traversal into a dedicated C++ `scl-dom` layer with source-location helpers.
2. Extend `DataTypeTemplates` resolution beyond direct `LNodeType -> DOType -> DA/DAType/BDA/EnumType` lookup: SDO, complete EnumVal metadata, full nested path expansion, and CDC-specific leaves.
3. Derive q/t metadata from resolved DO/DA definitions instead of fixture assumptions.
4. Add line/column or XPath location on top of the `pugixml` DOM traversal.
