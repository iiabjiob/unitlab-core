...
### Актуальный production startup guard

Deploy-путь теперь дожидается успешного завершения одноразового сервиса
`migrations` до запуска API и worker-сервисов. Это закрывает совместимость
старой БД с startup recovery sweep для `hardware_command_intents`: при
неприменённой схеме backend не должен начинать работу. Ручной запуск только
`backend` не является полноценным deploy-путём; сначала требуется
`docker compose up migrations`.

Миграционная история также сброшена: legacy-цепочка удалена, актуальная
схема создаётся единственным baseline `20260914_initial_schema`. Это требует
полного удаления disposable PostgreSQL volume перед первым запуском новой
версии; данные FAT, отчёты и evidence перед таким reset должны быть отдельно
сохранены или признаны ненужными.
