...
- External MMS discovery can start from only host/port: the native client first asks the live VMD logical-device list and then discovers the selected live domain.
...
| Multi-domain discovery | Root discovery selects the first live logical-device domain. | Extend the session model and backend contract to retain and expose every discovered domain, then allow RCB/DataSet selection across domains. |
