...
Для нового запроса действует общая пауза: запись с execution `unknown`/`timeout`
блокирует канал только первые 60 секунд от создания команды. Это правило едино
для ручного управления, импульсов, сиквенсов и автотестов. После паузы новая
операция проходит обычные проверки доступности и владения каналом; старый тест
не возобновляется автоматически, история не переписывается. Outbound worker
не отправляет просроченные или завершённые tracked-команды из очереди. Ошибка
восстановления после полученного ACK не считается потерей ACK и сохраняет
свою блокировку. Подробности: [DO command diagnostics](../guide/do-command-diagnostics.md).
