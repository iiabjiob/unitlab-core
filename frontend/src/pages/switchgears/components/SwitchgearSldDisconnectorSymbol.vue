...
<template>
  <g class="switchgear-sld-node-symbol switchgear-sld-node-symbol--disconnector" fill="none" stroke-linecap="round" stroke-linejoin="round" :stroke-dasharray="props.unconfigured ? '4 3' : undefined">
    <rect :x="props.x + 1" :y="props.y + 1" :width="props.width - 2" :height="props.height - 2" rx="8" :fill="props.background" stroke="none" />
    <g :transform="`rotate(${props.rotation ?? 0} ${props.x + props.width / 2} ${props.y + props.height / 2})`">
    ...
    <text v-if="props.state === 'UNKNOWN'" :x="props.x + props.width / 2" :y="props.y + props.height / 2 + 7" text-anchor="middle" font-size="22" font-weight="700" :fill="props.stroke" stroke="none">?</text>
  </g>
</template>
