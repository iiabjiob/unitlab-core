...
static int test_fixture_backed_discover_sequence_preserves_health_structure(void)
{
    DiscoveryFixtureHarness harness;
    UnitLabNativeClientSessionState session;
    UnitLabNativeDiscoveryIo io;
    ...
    if (!expect_true(unitlab_build_ied_model_plan(&fixture, &harness.plan, (char[256U]){0}, 256U) == 1, "expected Health discovery plan to build")) {
        return 0;
    }
    unitlab_mms_server_runtime_init(&harness.runtime);
    if (!expect_true(unitlab_mms_server_runtime_apply_model_plan(&harness.runtime, &harness.plan) == 1, "expected Health discovery plan to apply")) {
        unitlab_free_ied_model_plan(&harness.plan);
        return 0;
    }
    harness.config.bind_address = "127.0.0.1";
    harness.config.port = 15121;
    if (!expect_true(unitlab_mms_server_runtime_prepare(&harness.runtime, &harness.config, &harness.diagnostic) == 1, "expected Health discovery prepare to succeed")) {
        unitlab_free_ied_model_plan(&harness.plan);
        return 0;
    }
    if (!expect_true(unitlab_mms_server_runtime_start(&harness.runtime, &harness.diagnostic) == 1, "expected Health discovery start to succeed")) {
        unitlab_free_ied_model_plan(&harness.plan);
        return 0;
    }
    if (!expect_true(unitlab_mms_session_begin_association(&harness.runtime.session, &harness.diagnostic) == 1, "expected Health discovery session begin to succeed")) {
        unitlab_free_ied_model_plan(&harness.plan);
        return 0;
    }
    if (!expect_true(unitlab_mms_session_complete_association(&harness.runtime.session, 1U, &harness.diagnostic) == 1, "expected Health discovery session complete to succeed")) {
        unitlab_free_ied_model_plan(&harness.plan);
        return 0;
    }
    ...
    harness.response_capacity = sizeof(harness.response);
    g_discovery_harness = &harness;
    passed &= expect_true(unitlab_native_client_run_discover_sequence(&session, &io, "PROT", 100U, &next_invoke_id) == 1, "expected Health discovery sequence to succeed");
    g_discovery_harness = NULL;

    passed &= expect_true(session.discovered_data_name_count >= 1U, "expected at least one discovered data name for Health");
    passed &= expect_true(session.discovered_typed_data_node_count >= 4U, "expected Health structure to populate typed nodes");
    passed &= expect_true(session.discovered_leaf_ref_count >= 3U, "expected Health leaf refs to be captured");
