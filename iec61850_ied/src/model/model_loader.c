...
#ifdef UNITLAB_WITH_LIBIEC61850
    (void)config;
    if (!unitlab_validate_libiec61850_dynamic_model(fixture, plan, result)) {
        return 0;
    }
    set_result(
        result,
        "LIBIEC61850_SERVER_NOT_IMPLEMENTED",
        "libIEC61850 dynamic IED model, DataSets, and ReportControls were created, but MMS server startup is not implemented in this slice.");
    return 0;
#else
    (void)fixture;
    (void)plan;
