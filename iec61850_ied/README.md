...
- `discover <domain> [invokeBase]`
- `get-var-attrs <domain|-> <item> [invokeId]`
- `get-nvl-attrs <domain|-> <item> [invokeId]`
- `write-bool <domain> <item> <true|false|1|0> [invokeId]`
- `write-uint <domain> <item> <tag> <value> [invokeId]`
- `write-string <domain> <item> <tag> <value> [invokeId]`
- `write-hex <domain> <item> <tag> <hex> [invokeId]`

Commands emit confirmed responses as `wire-frame=<hex>` plus best-effort `mms-summary` decode lines, including GetNameList identifiers, GVA components, GNVLA members, and Read/Write access-result value hints. Write commands also emit an immediate post-write frame when the server sends one, for example a GI information report after `GI=true`. For IEDScout-style top-level LD browse use `get-name-list 9 0 - -`; for a compact native discover sequence that follows up to four browsed LNs/BRCBs/DataSets, uses class-1 domain LN directory pages, class-3 LN contents browse, reads discovered BRCB values, and follows DataSet `moreFollows` pages within that cap use `discover IED1LD0`; for domain DataSet browse use `get-name-list 2 1 <domain> -`; for attributes use `get-var-attrs IED1LD0 'LLN0$BR$brcbEvents$RptEna'` and `get-nvl-attrs LD0 'LLN0$dsEvents'`; for RCB enable/GI use `write-bool IED1LD0 'LLN0$BR$brcbEvents$RptEna' true` then `write-bool IED1LD0 'LLN0$BR$brcbEvents$GI' true`. RCB typed write examples: `write-uint IED1LD0 'LLN0$BR$brcbEvents$ResvTms' 5 30`, `write-hex IED1LD0 'LLN0$BR$brcbEvents$OptFlds' 4 061100`, `write-hex IED1LD0 'LLN0$BR$brcbEvents$TrgOps' 4 020c`, `write-string IED1LD0 'LLN0$BR$brcbEvents$DatSet' 10 IED1LD0/LLN0$dsEvents`, `write-string IED1LD0 'LLN0$BR$brcbEvents$RptID' 10 events`.
