...
Then in another terminal run `Start wire` and `Emit wire report` in the debug page. `Start wire` now sends a standards-shaped association request with session `CONNECT` framing and waits for the server's association response. The native server response now uses the matching session `ACCEPT` wrapper with the AARE/MMs initiate payload instead of the older `DATA_TRANSFER` shortcut. `Emit wire report` then sends a real unconfirmed information report frame from the simulator. Wireshark should decode the lower ISO-on-TCP layers, then show the association exchange followed by the report frame. Stop the capture with `Ctrl+C` after the test.
