...
void unitlab_mms_wire_association_fixture_init(UnitLabMmsWireAssociationFixture* fixture)
{
    ...
    unitlab_mms_session_spdu_init(&fixture->session);
...
int unitlab_mms_wire_association_fixture_decode(UnitLabMmsWireAssociationFixture* fixture, const uint8_t* buffer, size_t buffer_length, size_t* consumed_length, UnitLabMmsDiagnostic* diagnostic)
{
    ...
    if (fixture->transport.cotp.user_data_length == 0U || fixture->transport.cotp.user_data == NULL) {
        wire_association_fixture_set_diagnostic(diagnostic, UNITLAB_MMS_DIAGNOSTIC_PROTOCOL_ERROR, "association fixture is missing session bytes.");
        return 0;
    }
    if (!unitlab_mms_session_spdu_decode(&fixture->session, fixture->transport.cotp.user_data, fixture->transport.cotp.user_data_length, &session_consumed_length, diagnostic)) {
        return 0;
    }
    if (session_consumed_length != fixture->transport.cotp.user_data_length) {
        wire_association_fixture_set_diagnostic(diagnostic, UNITLAB_MMS_DIAGNOSTIC_PROTOCOL_ERROR, "association fixture contains trailing session bytes.");
        return 0;
    }
    if (fixture->session.raw_parameter_length == 0U || fixture->session.raw_parameter_bytes == NULL) {
    ...
    if (!unitlab_mms_presentation_decode(&fixture->presentation, fixture->session.raw_parameter_bytes, fixture->session.raw_parameter_length, &presentation_consumed_length, diagnostic)) {
        return 0;
    }
    if (presentation_consumed_length != fixture->session.raw_parameter_length) {
