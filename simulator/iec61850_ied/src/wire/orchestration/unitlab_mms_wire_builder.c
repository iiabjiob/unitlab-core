    ...
    UnitLabMmsDiagnostic* diagnostic)
{
    ...
    if (response_pdu == NULL || scratch == NULL || buffer == NULL || encoded_length == NULL) {
        wire_builder_set_diagnostic(diagnostic, UNITLAB_MMS_DIAGNOSTIC_INVALID_ARGUMENT, "Confirmed response frame requires a response PDU, scratch buffer, buffer, and encoded_length.");
        return 0;
    }
    ...
    if (scratch_length == 0U || buffer_length == 0U) {
        wire_builder_set_diagnostic(diagnostic, UNITLAB_MMS_DIAGNOSTIC_BUFFER_TOO_SMALL, "Confirmed response frame scratch and buffer must be non-zero.");
        return 0;
    }

    if (!unitlab_mms_pdu_encode(response_pdu, scratch, scratch_length, &response_payload_length, diagnostic)) {
        return 0;
    }
