    ...
    UnitLabMmsDiagnostic* diagnostic)
{
    ...
    if (response_pdu == NULL || buffer == NULL || encoded_length == NULL) {
        wire_builder_set_diagnostic(diagnostic, UNITLAB_MMS_DIAGNOSTIC_INVALID_ARGUMENT, "Confirmed response frame requires a response PDU, buffer, and encoded_length.");
        return 0;
    }
    ...
    if (buffer_length == 0U) {
        wire_builder_set_diagnostic(diagnostic, UNITLAB_MMS_DIAGNOSTIC_BUFFER_TOO_SMALL, "Confirmed response frame buffer is too small.");
        return 0;
    }
    ...
    if (response_payload == NULL) {
        wire_builder_set_diagnostic(diagnostic, UNITLAB_MMS_DIAGNOSTIC_BUFFER_TOO_SMALL, "Confirmed response scratch allocation failed.");
        return 0;
    }
