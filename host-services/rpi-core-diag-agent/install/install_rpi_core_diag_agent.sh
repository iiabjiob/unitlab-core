...
DEST_DIR="/opt/unitlab/rpi-core-diag-agent"
ENV_DIR="/etc/unitlab"
ENV_FILE="${ENV_DIR}/rpi-core-diag-agent.env"
UNIT_FILE="unitlab-rpi-core-diag-agent.service"
...
if [[ ! -f "${ENV_FILE}" ]]; then
  cat > "${ENV_FILE}" <<'EOF'
UNITLAB_CORE_DIAG_AGENT_REDIS_URL=redis://127.0.0.1:6379/0
UNITLAB_CORE_DIAG_AGENT_LOG_LEVEL=INFO
UNITLAB_CORE_DIAG_AGENT_STATUS_PUBLISH_INTERVAL_SEC=10
EOF
fi
