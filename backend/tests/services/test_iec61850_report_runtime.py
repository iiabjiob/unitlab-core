...
def test_backend_runtime_subscription_plan_runner_reuses_one_session_for_multiple_reports_on_same_endpoint() -> None:
    first_candidate = replace(
        _candidate(id="report-1"),
        report_control_name="brcbEventsA",
        rpt_id="IED1LD0/LLN0.BR.EventsA",
        data_set_ref="IED1/AP1/LD0/LLN0.dsEventsA",
    )
    second_candidate = replace(
        _candidate(id="report-2"),
        report_control_name="brcbEventsB",
        rpt_id="IED1LD0/LLN0.BR.EventsB",
        data_set_ref="IED1/AP1/LD0/LLN0.dsEventsB",
    )
    ...
    connect_events = [event for event in run.event_log if event.kind == "connect"]
    disconnect_events = [event for event in run.event_log if event.kind == "disconnect"]

    assert len(run.reports) == 2
    assert len(connect_events) == 1
    assert len(disconnect_events) == 1
    assert connect_events[0].session_id == disconnect_events[0].session_id
    assert connect_events[0].endpoint_id == "sim:IED1/AP1"
    assert {report.endpoint_id for report in run.reports} == {"sim:IED1/AP1"}
