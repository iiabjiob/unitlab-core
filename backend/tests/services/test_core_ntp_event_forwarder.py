...
@pytest.mark.anyio
async def test_state_clock_step_marks_presence_grace(monkeypatch: pytest.MonkeyPatch) -> None:
    redis = FakeRedis()
    mark_grace = AsyncMock()
    publish = AsyncMock()
    monkeypatch.setattr(forwarder, "mark_clock_adjustment_grace", mark_grace)
    monkeypatch.setattr(forwarder.WsEventPublisher, "publish", publish)  # pyright: ignore[reportPrivateLocalImportUsage]
    monotonic_calls = 0
    ...
    monkeypatch.setattr(forwarder.time, "monotonic", monotonic)  # pyright: ignore[reportPrivateLocalImportUsage]
    ...
    await forwarder._process_entries(  # pyright: ignore[reportPrivateUsage]
        cast(RedisStreamClient, cast(object, redis)),
        cast(RedisStreamEntries, cast(object, entries)),
    )
