...
class ChannelRepository:
    """Repository for CRUD operations on Channel."""
    ...
    async def get_by_device(self, device_id: int) -> list[Channel]:
        """Возвращает все каналы устройства."""
        result = await self.db.execute(select(Channel).where(Channel.device_id == device_id))
    ...
    async def register_or_update(
        ...
        # 3. Обновляем тип существующих каналов (если изменился)
