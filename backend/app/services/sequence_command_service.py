...
class SequenceCommandService:
    ...
    async def enqueue_start(sequence_id: int, requested_by: Optional[str] = None) -> SequenceCommand:
        command = SequenceCommand(type=SequenceCommandType.START, sequence_id=sequence_id, requested_by=requested_by)
    ...
    async def enqueue_stop(sequence_id: int, requested_by: Optional[str] = None) -> SequenceCommand:
        command = SequenceCommand(type=SequenceCommandType.STOP, sequence_id=sequence_id, requested_by=requested_by)
        await enqueue_sequence_command(command)
        return command
