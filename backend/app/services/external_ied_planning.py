...
def _json_value_list(value: object) -> list[object]:
    return cast(list[object], value) if isinstance(value, list) else []


class ExternalIedPlanningRedisClient(RedisHashClient, Protocol):
    async def smembers(self, name: str) -> set[str]: ...

    async def xadd(self, *args: object, **kwargs: object) -> str: ...


def _redis_client() -> ExternalIedPlanningRedisClient:
    return cast(ExternalIedPlanningRedisClient, cast(object, RedisManager.get_instance()))
...
class ExternalIedPlanningRequest:
    ...
    @classmethod
    def from_payload(cls, payload: JsonObject) -> ExternalIedPlanningRequest:
        ...
        return cls(
            ...
            workspace_id=_int_or_none(payload.get("workspace_id")) or 0,
            endpoint=endpoint,
            ip=ip,
            port=port,
...
async def load_external_ied_planning_endpoint_fields(workspace_id: int, endpoint: str) -> JsonObject:
    redis = _redis_client()
...
async def _load_discovery_cache(redis: RedisHashClient, *, workspace_id: int, endpoint: str) -> DiscoveryCache | None:
    ...
    fcdas = tuple(
        ...
        for item in _json_object_list(model.get("fcdas"))
        if str(item.get("reference") or "").strip()
    )
    datasets = tuple(
        DiscoveryCacheDataset(
            reference=str(item.get("reference") or ""),
            members=tuple(str(member) for member in _json_value_list(item.get("members")) if str(member).strip()),
        )
        for item in _json_object_list(model.get("datasets"))
        if str(item.get("reference") or "").strip()
    )
    rcbs = tuple(
        DiscoveryCacheRcb(
        ...
        for item in _json_object_list(model.get("rcbs"))
        if str(item.get("reference") or "").strip() and str(item.get("dataset_reference") or "").strip()
    )
...
def _signal_result_from_plan_signal(endpoint: str, signal: VerificationPlanSignal) -> JsonObject:
...
def _endpoint_payload_from_plan(
    ...
    signal_results: Sequence[JsonObject],
) -> JsonObject:
    ...
    return {
        ...
        "signal_ids": sorted(int(str(result["signal_id"])) for result in signal_results),
        "last_error": None,
    }
...
async def _store_planning_endpoint(
    ...
    *,
    ...
    _ = await redis.hset(_planning_state_key(workspace_id), endpoint, json.dumps(payload, separators=(",", ":"), sort_keys=True))
...
async def _remove_signal_results(
    redis: RedisHashClient,
    *,
    workspace_id: int,
    signal_ids: Sequence[int],
    ...
    for signal_id in signal_ids:
        _ = await redis.hdel(_planning_signal_key(workspace_id), str(signal_id))


async def _store_verification_plan(
    redis: RedisHashClient,
    *,
    workspace_id: int,
    endpoint: str,
    ...
    _ = await redis.hset(_planning_plan_key(workspace_id), endpoint, json.dumps(_plan_payload(plan), separators=(",", ":"), sort_keys=True))


async def _signal_ids_for_endpoint(
    redis: RedisHashClient,
    *,
    workspace_id: int,
    endpoint: str,
...
def _plan_payload(plan: VerificationPlan) -> JsonObject:
    return {
...
def _verification_enabled(metadata: JsonObject) -> bool:
    return _json_object(metadata.get("verification")).get("enabled") is True
...
def _transport_host_from_row(row: SignalAllocationRowSchema) -> str | None:
    ...
    source_value = metadata.get("row")
    verification = _json_object(verification_value)
    ...
    for source_map in (verification, source, metadata):
        for key in ("transport_host", "transport_reference", "host", "ip_address", "device_ip", "target_ip", "mms_host", "endpoint_host", "ip"):
            value = _str_or_none(source_map.get(key))
            if value:
                return value
    return None
...
def _normalize_port(value: object) -> int:
    port = int(str(value or 102))
    if port < 1 or port > 65535:
        return 102
    return port


def _normalize_signal_ids(value: object) -> tuple[int, ...]:
    if not isinstance(value, list | tuple | set):
        return ()
    result = sorted({int(str(item)) for item in cast(list[object] | tuple[object, ...] | set[object], value) if _int_or_none(item) and int(str(item)) > 0})
    return tuple(result)


def _planning_reason(value: object, *, event: str) -> PlanningRequestReason:
    if value in {"mapping_changed", "verification_required", "user_refresh", "discovery_completed"}:
        return cast(PlanningRequestReason, value)
    if event == "ExternalIedDiscoveryCompleted":
        return "discovery_completed"
    return "mapping_changed"


def _rcb_kind(value: object) -> Literal["buffered", "unbuffered", "unknown"]:
    text = str(value or "").strip().lower()
    if text in {"buffered", "unbuffered"}:
        return cast(Literal["buffered", "unbuffered"], text)
    return "unknown"


def _parse_payload(payload: str | bytes | JsonObject | None) -> JsonObject | None:
