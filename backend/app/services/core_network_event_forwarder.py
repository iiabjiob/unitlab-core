...
from app.schemas.ws.events import CoreNetworkStateEvent

settings = get_settings()
logger = get_logger("core.net.forwarder")

STREAM_NAME = settings.core_net_event_stream
GROUP_NAME = "core-net-events"
...
async def _ensure_group(redis) -> None:
    try:
        ...
        logger.info("✅ Created consumer group %s for core network events", GROUP_NAME)
    except ResponseError as exc:
        if "BUSYGROUP" in str(exc):
            logger.info("ℹ️ Core network event consumer group already exists")
        else:
            raise
...
async def _drain_pending(redis) -> None:
    while True:
        ...
        if not entries:
            break
        logger.info("🔁 Replaying %d pending core network events", len(entries))
...
async def _process_entries(redis, entries) -> None:
    for entry_id, fields in entries:
        try:
            ...
            if not payload:
                logger.warning("⚠️ Invalid core network event payload in %s", entry_id)
            ...
            logger.exception("💥 Failed to forward core network event %s: %s", entry_id, exc)
        finally:
            await redis.xack(STREAM_NAME, GROUP_NAME, entry_id)


async def forward_core_network_events() -> None:
