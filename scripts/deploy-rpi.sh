...
Environment overrides:
  RELEASES_DIR, CURRENT_LINK, COMPOSE_FILE_NAME, VERIFY_RUNTIME, CLEANUP_RELEASES
EOF
}
...
if [[ -n "$previous_target" && "$target_release" == "$previous_target" ]]; then
  echo "[unitlab] ERROR: trying to overwrite active release: $target_release" >&2
  exit 1
fi
...
if [[ -n "$IMAGES_ARCHIVE" ]]; then
  ...
  docker compose --project-directory "$CURRENT_LINK" -f "$compose_file" up -d --remove-orphans
else
  echo "[unitlab] Pulling images and starting stack"
  docker compose --project-directory "$CURRENT_LINK" -f "$compose_file" pull
  docker compose --project-directory "$CURRENT_LINK" -f "$compose_file" up -d --remove-orphans
fi
...
if [[ "$VERIFY_RUNTIME" == "1" ]]; then
  ...
  if ! "$CURRENT_LINK/scripts/verify-rpi-runtime.sh" --project-dir "$CURRENT_LINK" --compose-file "$compose_file" "${verify_args[@]}"; then
    ...
    if [[ -n "$previous_target" && -d "$previous_target" ]]; then
      ...
      load_release_env "$CURRENT_LINK"
      docker compose --project-directory "$CURRENT_LINK" -f "$CURRENT_LINK/$COMPOSE_FILE_NAME" up -d --remove-orphans
